...
## 19. Pages secondaires — référence actuelle

La [cartographie détaillée de la page Strain et le contrat commun des pages secondaires](design/SECONDARY_PAGE_BLUEPRINT.md) définissent le gabarit actuel pour Strain, Alimentation, Sommeil et Récupération. Ils fixent les positions, dimensions, couleurs, polices, espacements, interactions, animations, états et règles d’adaptation, avec les différences métier autorisées. Le titre de page est le seul titre de section visible par défaut ; les groupes de données gardent un nom accessible. Le radar et le score partagent une hiérarchie commune, les indicateurs suivent la synthèse, et les graphiques restent sans cartes ni titres décoratifs.

Cette cartographie part de la version locale de la page Strain validée le 24 septembre 2026, branche `codex/activity-annotations-20260924`, commit `644449f`. Elle sert de cible documentaire pour les autres pages ; elle ne signifie pas que celles-ci sont déjà uniformisées ni que cette branche est déjà intégrée à `main`. Les règles de données, sécurité, accessibilité et thème des sections 1 à 18 restent applicables.

L’[ancien contrat de septembre](design/SECONDARY_PAGES_2026-09_ARCHIVE.md) est conservé comme archive et n’a plus de portée normative lorsqu’il diverge de cette référence.
