...
import { cloudflareDb, createCloudflareAdminClient, hasSupabaseRuntime, sessionUserForHash } from "@/lib/cloudflare/db";
...
export async function createSession(userId: string) {
  ...
  if (hasSupabaseRuntime()) {
    ...
    return { token, cookieOptions, session: { id: sessionId, platform, deviceName, createdAt: now.toISOString(), expiresAt: expires.toISOString() } satisfies WebSession };
  }
...
async function getSessionUserForToken(token: string | null): Promise<SessionUser | null> {
  ...
  if (hasSupabaseRuntime()) {
    try {
      const user = await sessionUserForHash(await sha256(token), new Date().toISOString(), SESSION_READ_TIMEOUT_MS);
      if (!user) return null;
      return {
        id: user.id,
        email: typeof user.email === "string" ? user.email : null,
        displayName: typeof user.display_name === "string" ? user.display_name : "Soma user",
      };
    } catch {
