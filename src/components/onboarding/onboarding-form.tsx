...
export function OnboardingForm({ demoMode }: { demoMode: boolean }) {
  ...
  function goToStep(nextStep: number) {
    ...
    window.requestAnimationFrame(() => {
      ...
      document.querySelector(".onboarding-main")?.scrollIntoView({ block: "start", behavior: "smooth" });
    });
  }
  ...
  function continueToNextStep() {
    ...
    const currentStepValid = step !== 0 || Boolean(
      ...
      Number.isFinite(form.heightCm) && form.heightCm >= 50 && form.heightCm <= 260 &&
      Number.isFinite(form.weightKg) && form.weightKg >= 20 && form.weightKg <= 400,
    );
