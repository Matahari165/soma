...
export function LabGlobalNavigation() {
  ...
  return <nav className="lab-global-nav" aria-label="Navigation principale">
    <Link href="/" prefetch={false} onClick={(event) => { if (pathname !== "/") return; event.preventDefault(); window.history.replaceState(null, "", "/"); window.scrollTo({ top: 0, behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "instant" : "smooth" }); }}>Personal Lab</Link>
