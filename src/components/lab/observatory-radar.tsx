...
export function ObservatoryRadar({data, date, radius = DEFAULT_RADAR_RADIUS, shiftX = 0, shiftY = 0, detailId: detailIdProp, selectedId: selectedIdProp, onSelect: onSelectProp}:{data:RadarData; date?:string; radius?:number; shiftX?:number; shiftY?:number; detailId?:string; selectedId?:string|null; onSelect?:(id:string|null)=>void}) {
  ...
  const axes=[
    ...
    {id:"effort",label:"Effort",average:averageEffortScore,value:effortScore,target:100,unit:"",display:effortScore===null?"—":`${Math.round(effortScore)}`,goal:"100 / 100",source:"Soma",definition:"Score d’activité quotidien calculé par Soma.",readingDirection:"100 = tous les objectifs quotidiens atteints",role:"Indicateur du jour",formula:"score d’activité Soma",normalization:"0–100"},
