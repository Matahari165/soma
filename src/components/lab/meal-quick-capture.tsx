...
type SlotState = { state: CaptureState; file: File | null; origin: MealOrigin | null; message: string | null; photoCount: number };
...
const emptySlot = (): SlotState => ({ state: "idle", file: null, origin: null, message: null, photoCount: 0 });
...
export function MealQuickCapture({ todayDate, variables, entries, days, breakfastDisabledOverride }: { todayDate: string; variables: JournalVariable[]; entries: JournalEntry[]; days: JournalDay[]; breakfastDisabledOverride?: boolean }) {
  ...
  async function submit(slot: MealType, origin: MealOrigin) {
    ...
    try {
      ...
      setSlotStates((current) => ({ ...current, [slot]: { state: "done", file: null, origin: null, message: "Photo ajoutée · analyse prête", photoCount: current[slot].photoCount + 1 } }));
    } catch (error) {
  ...
  return <section className="meal-quick" aria-labelledby="meal-quick-title">
    <header><div><span className="eyebrow">Repas aujourd’hui</span><h2 id="meal-quick-title">Photos des repas</h2></div><a href="/meals">Voir l’historique</a></header>
