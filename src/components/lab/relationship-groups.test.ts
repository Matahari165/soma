...
describe("groupMatrixRows", () => {
  ...
  it("keeps running and sedentary influences in one coherent group", () => {
    const group = influenceGroup("running_distance");
    expect(["sedentary_minutes", "running_pace", "running_average_heart_rate", "vo2_max"].every((id) => influenceGroup(id) === group)).toBe(true);
    expect(group).toBe("Running & sedentary time");
  });
