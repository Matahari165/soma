...
export function LabWorldWorkspace({ date, metrics, effects, capture }: {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => entries.forEach(entry => {
      ...
      element.animate([{ opacity: .35, transform: theme === "index" ? "translateX(-16px)" : theme === "focus" ? "scale(.975)" : "translateY(18px)" }, { opacity: 1, transform: "none" }], { duration: theme === "index" ? 400 : 650, easing: "cubic-bezier(.2,.7,.2,1)" });
