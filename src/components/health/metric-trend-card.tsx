...
export function MetricTrendCard({ label, points, unit, direction, format = defaultFormat, target, href, displayDays = 30 }: {
  ...
  const cardContent = <>
    ...
    <footer><span>Latest {formatDate(trend.currentDate ?? undefined)} · {completeCount} measured days</span>{href && <ArrowUpRight className="metric-card-cue" size={17} aria-hidden="true" />}</footer>
  </>;
