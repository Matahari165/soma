...
import { DAILY_STRAIN_WEIGHTS, dailyStrainProgress, calculateDailyStrain, activeHoursFromScoreRow, dailyStrainScoreFromRow, activityLoadFromScoreRow } from "@/domain/scores/effort";
...
export function effortComponentDefinitions(): readonly EffortComponentDefinition[] {
  return [
    { id: "zoneMinutes", label: "Minutes en zone", target: 45, targetLabel: "45 min", weight: DAILY_STRAIN_WEIGHTS.zoneMinutes, unit: "min", key: "zone_minutes", formula: "Minutes en zone / 45, puis dépassement valorisé à 60 %, bonus maximal de 50 %" },
    { id: "strengthMinutes", label: "Renforcement", target: 10, targetLabel: "10 min", weight: DAILY_STRAIN_WEIGHTS.strengthMinutes, unit: "min", key: "strength_minutes", formula: "Renforcement enregistré / 10 min, puis dépassement valorisé à 60 %, bonus maximal de 50 %" },
    { id: "activeHoursProgress", label: "Heures actives", target: 1, targetLabel: "100 % des heures", weight: DAILY_STRAIN_WEIGHTS.activeHoursProgress, unit: "%", key: "active_hours", formula: "Heures actives / heures éveillées écoulées ; couverture complète requise" },
    { id: "steps", label: "Pas", target: 10_000, targetLabel: "10 000 pas", weight: DAILY_STRAIN_WEIGHTS.steps, unit: "pas", key: "steps", formula: "Pas / 10 000, puis dépassement valorisé à 60 %, bonus maximal de 50 %" },
  ];
}
...
function componentValue(id: EffortComponentId, day: HealthMetricDay | undefined, row: ScoreDay | undefined) {
  ...
  if (row?.algorithm_version !== "effort-v5" && row?.algorithm_version !== "effort-v6") return null;
...
function activityScoreBreakdown(latest: HealthMetricDay | undefined, row: ScoreDay | undefined, definitions: readonly EffortComponentDefinition[]): ActivityScoreBreakdown {
  ...
  const components = values.map((component) => {
    ...
    const scoreNormalizedValue = component.value === null ? null : dailyStrainProgress(component.value, component.target, component.id === "activeHoursProgress");
    return { id: component.id, label: component.label, weight: component.weight, sourceValueLabel: component.id === "activeHoursProgress" ? component.value === null ? "Indisponible" : `${Math.round(component.value * 100)} %` : formatComponentValue(component.value, component.unit), targetLabel: component.targetLabel, normalizedValue, scoreNormalizedValue, contribution: scoreNormalizedValue === null ? null : scoreNormalizedValue * component.weight, formula: component.formula, normalization: component.id === "activeHoursProgress" ? "Heures validées, maximum 10 points" : "100 % jusqu’à l’objectif, puis 60 % ; maximum 45 points" };
  });
...
export function ActivityDetails({ data }: { data: HealthAnalytics }) {
  ...
  const radarDimensions: ActivityRadarDimension[] = latest ? effortComponents.map((component) => {
    ...
    return {
      ...
      readingDirection: "100% = daily goal reached", scoreRole: `Strain score component · ${component.weight}%`, scoreWeight: component.weight, scoreFormula: component.formula, scoreNormalization: breakdown?.normalization, scoreContribution: breakdown?.contribution ?? null, comparison: comparison(value, average), comparisonLabel: comparisonLabel(average, component.unit), comparisonTone: metricTone(value, average, "higher_is_better"),
      sourceLabel,
    };
  }) : [];
