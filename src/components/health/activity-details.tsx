    ...
    <HealthPageShell kind="activity" title="Activity" description="Activity score based on zone minutes, exercise duration, active energy, and steps." score={score} freshness={freshness} timezone={data.timezone} showFreshness={true} heroScore={<span className="sr-only">Activity score: {score === null ? "unavailable" : `${score} out of 100`}. 30-day average: {averages.effort === null ? "unavailable" : `${Math.round(averages.effort)} out of 100`}. {scoreCoverage === null ? "Score coverage unavailable" : `${Math.round(scoreCoverage * 100)}% score coverage`}.</span>}>
      <section className={`${styles.content} health-observatory-content`} aria-label="Activity content">
        {latest ? <>
          ...
          <section className={`${styles.section} health-observatory-panel`} aria-labelledby="activity-sessions-heading"><header className={styles.sectionHeader}><h2 id="activity-sessions-heading">Recent workouts</h2><span>{data.exercises.length} workout{data.exercises.length === 1 ? "" : "s"}</span></header>{data.exercises.length ? <div className={styles.exerciseTableWrap} role="region" aria-label="Recent workouts, horizontally scrolling table" tabIndex={0}><table className={styles.exerciseTable}><thead><tr><th>Workout</th><th>Date</th><th>Duration</th><th>Active time</th><th>Calories</th><th>Distance</th><th>Pace / speed</th><th>Avg HR</th><th>Zone min</th><th>Elevation</th></tr></thead><tbody>{data.exercises.map((exercise) => <tr key={exercise.id}><th scope="row"><Footprints size={15} aria-hidden="true" />{exercise.name}<small>{exerciseTypeLabel(exercise.type)}</small></th><td>{exercise.date}</td><td>{number(exercise.durationMinutes, "min")}</td><td>{number(exercise.activeMinutes, "min")}</td><td>{number(exercise.calories, "kcal")}</td><td>{measured(exercise.distanceKm) ? `${exercise.distanceKm.toFixed(2)} km` : "—"}</td><td>{paceOrSpeed(exercise) ?? "—"}</td><td>{number(exercise.averageHeartRate, "bpm")}</td><td>{number(exercise.zoneMinutes, "min")}</td><td>{number(exercise.elevationGainMeters, "m")}</td></tr>)}</tbody></table></div> : <p className={styles.empty}>No workouts recorded.</p>}</section>
        </> : <section className={`${styles.empty} health-observatory-panel`} aria-labelledby="activity-empty-heading"><Footprints size={24} aria-hidden="true" /><div><h2 id="activity-empty-heading">No activity data</h2><p>0 measured days over the last 30 days. Import your activity from Google Health, then return here.</p><p><a href="/settings">Check Google Health connection</a></p></div></section>}
      </section>
    </HealthPageShell>
  </div>;
}
