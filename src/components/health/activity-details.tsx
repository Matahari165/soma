...
export function ActivityDetails({ data }: { data: HealthAnalytics }) {
  ...
  const heroMetrics = latest ? <>
    <div className={`health-hero-stat metric-tone--${tones.activeCalories}`}><span>Active calories</span><AnimatedMetricReading value={latest.active_energy_kcal} format="number" decimals={0} unit={latest.active_energy_kcal === null ? undefined : "kcal"} className={`metric-reading--${tones.activeCalories}`} /><small className="health-hero-stat__average">30-day average · {formatAverage(averages.activeCalories, "number")} kcal</small></div>
    <div className="health-hero-stat metric-tone--neutral"><span>Zone minutes</span><AnimatedMetricReading value={latest.zone_minutes} format="number" decimals={0} unit={latest.zone_minutes === null ? undefined : "min"} className="metric-reading--neutral" /><small className="health-hero-stat__average">Latest complete day</small></div>
    <div className={`health-hero-stat metric-tone--${tones.weeklyLoad}`}><span>Weekly load</span><AnimatedMetricReading value={latest.weekly_load} format="number" decimals={0} className={`metric-reading--${tones.weeklyLoad}`} /><small className="health-hero-stat__average">30-day average · {formatAverage(averages.weeklyLoad, "number")}</small></div>
  </> : undefined;
  ...
  return <div className={styles.root}><HealthPageShell kind="activity" title="Effort" description="Movement, training load, and active days." score={score} freshness={freshness} timezone={data.timezone} heroScore={<div className={`activity-score-tone activity-score-tone--${tones.dailyLoad} ${styles.headerScoreMetric}`}><span>Effort score</span><div className={styles.headerScoreBody}><div><strong>{score === null ? "—" : Math.round(score)}</strong><small>%</small><p>30-day average · {averages.dailyLoad === null ? "—" : Math.round(averages.dailyLoad)}</p></div><div className={styles.headerScoreBars} aria-hidden="true">{recentEffortScores.map((item, index) => <i key={`effort-${index}`} style={{ height: item === null ? "20%" : `${effortMax === effortMin ? 58 : 28 + ((item - effortMin) / (effortMax - effortMin)) * 52}%` }} />)}</div></div><div className={styles.headerScoreAction}><ActivityScorePopover score={score} zoneMinutes={latest?.zone_minutes ?? null} exerciseMinutes={latest?.exercise_minutes ?? null} activeEnergyKcal={latest?.active_energy_kcal ?? null} steps={latest?.steps ?? null} /></div></div>} heroMetrics={heroMetrics}>
    {latest ? <>
