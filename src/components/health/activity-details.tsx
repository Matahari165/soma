  ...
  return <div className={styles.root}>
    <HealthPageShell kind="activity" title="Effort" description="Score d’effort fondé sur les minutes en zone, l’exercice, l’énergie active et les pas." score={score} freshness={freshness} timezone={data.timezone} showFreshness={true} heroScore={<span className="sr-only">Score d’effort : {score === null ? "indisponible" : `${score} sur 100`}. Moyenne sur 30 jours : {averages.effort === null ? "indisponible" : `${Math.round(averages.effort)} sur 100`}. {scoreCoverage === null ? "Couverture du score indisponible" : `${Math.round(scoreCoverage * 100)} % de couverture du score`}.</span>}>
      <section className={`${styles.content} health-observatory-content`} aria-label="Contenu effort">
        ...
        </> : <section className={`${styles.empty} health-observatory-panel`} aria-labelledby="activity-empty-heading"><Footprints size={24} aria-hidden="true" /><div><h2 id="activity-empty-heading">Aucune donnée d’effort</h2><p>0 journée mesurée sur les 30 derniers jours. Importez votre activité depuis Google Health, puis revenez ici.</p><p><a href="/settings">Vérifier la connexion Google Health</a></p></div></section>}
      </section>
    </HealthPageShell>
  </div>;
}
