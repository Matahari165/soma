...
export function LineTrendChart({ points, label, target }: { points: MetricPoint[]; label: string; target?: number | null }) {
  ...
  if (available.length < 2) return <p className="health-empty">Pas assez de mesures complètes pour afficher une tendance.</p>;
  ...
  return <div className="health-line-chart-wrap"><svg className="health-line-chart" viewBox="0 0 300 104" role="img" tabIndex={0} aria-label={`${label} : tendance sur ${available.length} jours mesurés. Les absences ne sont pas reliées. Utilisez les flèches gauche et droite pour parcourir les points.`} onFocus={() => setActivePoint(available.at(-1) ?? null)} onBlur={() => setActivePoint(null)} onKeyDown={(event) => {
  ...
  </svg><span className="health-chart-range" aria-hidden="true"><b>{max.toFixed(1)}</b><b>{min.toFixed(1)}</b></span>{activePoint && <output className="health-chart-tooltip" aria-live="polite">{new Date(activePoint.date).toLocaleDateString("fr-FR", { month: "short", day: "numeric" })} · {activePoint.value.toFixed(1)}</output>}</div>;
}
...
export function SleepStageTimeline({ stages }: { stages: SleepStageSegment[] }) {
  ...
  return <div className="sleep-stage-chart">
    ...
    <div className="sleep-stage-timeline" role="img" aria-label="Séquence des phases de sommeil pendant la dernière nuit">
      {stages.map((stage, index) => <span key={`${stage.startTime}-${index}`} className={`sleep-stage-segment sleep-stage-segment--${stageClass[stage.type]}`} style={{ flexGrow: safeDurations[index] / total }}><span className="sr-only">{{ AWAKE: "Éveillé", LIGHT: "Léger", DEEP: "Profond", REM: "REM", ASLEEP: "Léger", RESTLESS: "Agité" }[stage.type]}</span></span>)}
    </div>
  </div>;
}
...
export function SleepStageDistribution({ stages }: { stages: Array<{ label: string; value: number | null; tone: string }> }) {
  ...
  if (!available.length) return <p className="health-empty">La répartition des phases n’est pas disponible.</p>;
  return <div><div className="stage-distribution" role="img" aria-label={available.map((stage) => `${stage.label} ${stage.value.toFixed(1)} pour cent`).join(", ")}>
...
export function HeartRateCurve({ samples }: { samples: HeartRateSample[] }) {
  return <LineTrendChart points={samples.map((sample) => ({ date: sample.measuredAt, value: sample.bpm }))} label="Fréquence cardiaque" />;
}
...
export function ZoneDistribution({ zones }: { zones: Array<{ label: string; minutes: number | null; tone: string }> }) {
  ...
  if (!total) return <p className="health-empty">Aucun temps par zone cardiaque n’est disponible.</p>;
