...
export function SettingsConsole({ initialHealthNotice = null }: { initialHealthNotice?: GoogleHealthNotice | null }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/health/sync", { signal: controller.signal }).then((response) => response.ok ? response.json() : Promise.reject(new Error("Your sync status could not be loaded."))),
    ]).then(([profileData, connectionData, syncData]) => {
