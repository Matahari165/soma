...
export function CoachChat({ compact = false, initialThreadId = null, initialMessages, onThreadCreated }: { compact?: boolean; initialThreadId?: string | null; initialMessages?: ChatMessage[]; onThreadCreated?: (thread: { id: string; title: string }) => void }) {
  ...
  return <div className={compact ? "chat chat--compact" : "chat"}>
    ...
    <form className="chat-composer" onSubmit={(event) => { event.preventDefault(); void send(); }}><label className="sr-only" htmlFor={compact ? "coach-compact" : "coach-full"}>Message Soma Coach</label><textarea id={compact ? "coach-compact" : "coach-full"} rows={1} maxLength={4000} value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="Ask Soma…" aria-describedby={compact ? undefined : "coach-send-hint"} /><button disabled={!message.trim() || sending} type="submit" aria-label="Send message">{sending ? <LoaderCircle className="spin" size={18} /> : <Send size={18} />}</button>{!compact && <small id="coach-send-hint">Enter to send</small>}</form>
  </div>;
}
