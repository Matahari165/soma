...
export function CoachChat({ compact = false, initialThreadId = null, initialMessages, onThreadCreated }: { compact?: boolean; initialThreadId?: string | null; initialMessages?: ChatMessage[]; onThreadCreated?: (thread: { id: string; title: string }) => void }) {
  ...
  async function decide(messageId: string, proposalId: string | null | undefined, decision: "confirm" | "reject") {
    ...
    const result = await response.json();
    setMessages((current) => current.map((item) => item.id === messageId ? { ...item, actionStatus: response.ok ? result.status : result.error } : item));
  }

  return <div className={compact ? "chat chat--compact" : "chat"}>
    <div className="chat-messages" aria-live="polite">{messages.map((item) => <article className={`chat-bubble chat-bubble--${item.role}`} key={item.id}>{item.role === "assistant" && <span className="chat-avatar"><Sparkles size={15} /></span>}<div><p>{item.content}</p>{item.evidence?.length ? <ul className="evidence-list">{item.evidence.map((evidence) => <li key={evidence}>{evidence}</li>)}</ul> : null}{item.action && <div className="action-preview"><span>Preview · confirmation required</span><h3>{item.action.title}</h3><p>{item.action.description}</p>{item.actionStatus ? <strong>{item.actionStatus}</strong> : <div><button onClick={() => decide(item.id, item.proposalId, "reject")} type="button"><X size={15} />Reject</button><button className="primary" onClick={() => decide(item.id, item.proposalId, "confirm")} type="button"><Check size={15} />Confirm</button></div>}</div>}</div></article>)}{sending && <div className="chat-thinking"><LoaderCircle className="spin" size={16} /> Soma is checking your data…</div>}</div>
    ...
    <form className="chat-composer" onSubmit={(event) => { event.preventDefault(); void send(); }}><label className="sr-only" htmlFor={compact ? "coach-compact" : "coach-full"}>Message Soma Coach</label><textarea id={compact ? "coach-compact" : "coach-full"} rows={1} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Ask Soma about your health…" /><button disabled={!message.trim() || sending} type="submit" aria-label="Send message">{sending ? <LoaderCircle className="spin" size={18} /> : <Send size={18} />}</button></form>
  </div>;
}
