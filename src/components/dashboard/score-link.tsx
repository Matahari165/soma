...
export function ScoreLink({ metric, index }: { metric: DailyScore; index: number }) {
  ...
  return (
    ...
    >
      <span className="score-link__channel" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
      ...
      <span className="score-link__reading">
        <strong>{metric.score ?? "—"}</strong>
        <small>{metric.score === null ? "Awaiting complete inputs" : "/ 100"}</small>
      </span>
      <span className="score-link__plot">
        ...
        <small>Recent readings</small>
      </span>
      <span className="score-link__context"><strong>{metric.value}</strong><small>{metric.delta}</small></span>
