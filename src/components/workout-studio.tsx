...
export function WorkoutStudio() {
  ...
  const [setCompletedReps, setSetCompletedReps] = useState("");
  ...
  async function start(program: Program) {
    ...
    try {
      ...
      setSetWeightKg("");
      setSetCompletedReps(String(program.exercises[0]?.repsMin ?? ""));
    } catch (error) {
  ...
  async function completeSet() {
    ...
    const weightKg = setWeightKg.trim() === "" ? null : Number(setWeightKg);
    const completedReps = Number(setCompletedReps);
    if (setCompletedReps.trim() === "" || !Number.isInteger(completedReps) || completedReps < 0 || completedReps > 1000) {
      setActionError("Indique le nombre de répétitions réellement effectuées, entre 0 et 1 000.");
      return;
    }
    if (weightKg !== null && (!Number.isFinite(weightKg) || weightKg < 0 || weightKg > 10000)) {
      setActionError("Indique une charge entre 0 et 10 000 kg, ou laisse le champ vide.");
      return;
    }
    setUpdatingSession(true);
    setActionError(null);
    try {
      await patchSession({ action: "complete_set", exercisePosition: active.exerciseIndex, setIndex: active.setIndex, completedReps, weightKg });
      setSetWeightKg("");
      ...
      setSetCompletedReps(String(lastSet ? active.program.exercises[active.exerciseIndex + 1]?.repsMin ?? "" : currentExercise.repsMin));
