...
import { buildPreviewAnalytics } from "./health-analytics";
...
export function previewData() {
  ...
  const activityPreview = buildPreviewAnalytics();
  const activityDays = new Map(activityPreview.days.map((day) => [day.metric_date, day]));
  const activityScores = new Map(activityPreview.scores.filter((score) => score.kind === "effort").map((score) => [score.score_date, score]));
  ...
  for (let index = 0; index < 210; index += 1) {
    ...
    const activityDay = activityDays.get(dateString);
    const activityScore = activityScores.get(dateString);
    ...
    scores.push(
      ...
      { score_date: dateString, kind: "effort", score: activityScore ? activityScore.score : olderEffort?.score ?? null, drivers: { coverage: activityScore?.drivers.coverage ?? olderEffort?.coverage ?? null, activityLoadScore: activityScore ? activityScore.drivers.activityLoadScore : olderEffort?.loadScore ?? null } },
    );
