...
describe("durable meal photo upload cleanup", () => {
  const mealId = "12345678-1234-1234-1234-123456789012";

  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(claimCloudflareLockWithToken).mockResolvedValue("lease-token");
    vi.mocked(releaseCloudflareLockWithToken).mockResolvedValue(undefined);
    state.findMeal.mockResolvedValue({ id: mealId, userId: "user-1", status: "draft", photos: [] });
    state.createMealPhotoUploadJob.mockResolvedValue(undefined);
    state.deleteMealPhotoUploadJob.mockResolvedValue(undefined);
    vi.mocked(mealPhotoObjectPath).mockReturnValue("meal-photos/user-1/meal-1/photo.jpg");
  });

  it("records the key before R2 and retains it when orphan cleanup fails", async () => {
    const steps: string[] = [];
    state.createMealPhotoUploadJob.mockImplementation(async () => { steps.push("job"); });
    vi.mocked(putR2MealPhotoObject).mockImplementation(async () => { steps.push("r2"); });
    state.insertPhoto.mockRejectedValue(new Error("Database unavailable"));
    vi.mocked(deleteR2MealPhotoObject).mockRejectedValue(new Error("R2 unavailable"));

    await expect(addMealPhotos("user-1", mealId, [{ mimeType: "image/jpeg", size: 1, data: new Uint8Array([1]).buffer, origin: "unknown" }]))
      .rejects.toMatchObject({ code: "unavailable" });

    expect(steps).toEqual(["job", "r2"]);
    expect(state.deleteMealPhotoUploadJob).not.toHaveBeenCalled();
  });

  it("reconciles only stale tracked uploads without a saved photo", async () => {
    ...
    state.findMealPhoto.mockResolvedValue(null);
