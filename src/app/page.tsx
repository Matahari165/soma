...
export default async function TodayPage({ searchParams }: { searchParams: Promise<{ health?: string | string[]; calendar?: string | string[] }> }) {
  ...
  if (isLocalPreviewMode() || isObservatoryMode()) return <Suspense fallback={<div id="main-page-content" className="lab-world-loading" role="status">Chargement du laboratoire…</div>}><LabWorldPreview stream={stream} /></Suspense>;
