...
import { COLOR_SCHEME_COLORS, COLOR_SCHEME_INIT_SCRIPT } from "@/lib/color-scheme";
...
import "./color-scheme-backdrop.css";
...
export const viewport: Viewport = {
  ...
  initialScale: 1,
  colorScheme: "light",
  themeColor: COLOR_SCHEME_COLORS.light,
};
...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <html className={`${schibsted.variable} ${azeretMono.variable}`} lang="en" data-color-scheme="light" data-lab-theme={darkInterface ? "observatory" : undefined} data-scroll-behavior="smooth" suppressHydrationWarning>
      <head><script id="soma-color-scheme-init" nonce={nonce} dangerouslySetInnerHTML={{ __html: COLOR_SCHEME_INIT_SCRIPT }} /></head>
      <body className={[localPreview && "local-preview", observatoryMode && "observatory-mode"].filter(Boolean).join(" ") || undefined}>
        <SkipLink />
        {localPreview && <div className="preview-banner" role="status"><strong>{isRemoteDemoPreviewMode() ? "TEST PREVIEW" : "LOCAL PREVIEW"}</strong><span>{isRemoteDemoPreviewMode() ? "Données de démonstration · Consultation uniquement" : "Données de démo · Les messages du chat sont envoyés à OpenAI, sans sauvegarde locale durable"}</span></div>}
