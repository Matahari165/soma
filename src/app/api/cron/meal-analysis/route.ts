...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [requeued, purgedExpired, purgeRetry] = await Promise.all([
    ...
    if (requeued > 0 || purgedExpired > 0 || purgeRetry.attempted > 0) Object.assign(body, { requeued, purgedExpired, purgeRetry });
