...
vi.mock("@/app/api/meals/[id]/route", () => ({ GET: vi.fn(), PATCH: vi.fn() }));
...
describe("native meal API Bearer boundary", () => {
  ...
  it("rejects a Web cookie on every native meal route", async () => {
    ...
    const responses = await Promise.all([
      ...
      getMeal(request(), mealContext), patchMeal(request(), mealContext),
    ...
    expect(responses.map((response) => response.status)).toEqual(Array(11).fill(401));
