...
function serviceError(error: unknown) {
  if (!(error instanceof SupplementServiceError)) {
    return NextResponse.json({ error: "La prise de complément est momentanément indisponible." }, { status: 500, headers: noStore });
  }
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return entry
      ? NextResponse.json({ entry: supplementEntryToView(entry) }, { headers: noStore })
      : NextResponse.json({ error: "Prise de complément introuvable.", code: "not_found" }, { status: 404, headers: noStore });
  } catch (error) {
