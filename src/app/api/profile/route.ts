...
export async function GET() {
  ...
  const [profileResult, sleepResult, goal] = await Promise.all([
    ...
    loadActiveGoal(user.id),
  ]);
  const failure = profileResult.error ?? sleepResult.error;
  if (failure) {
    console.error("[api/profile] load failed", { code: failure.code });
    return NextResponse.json({ error: "Your profile could not be loaded." }, { status: 500 });
  }
  const profile = profileResult.data;
  const sleep = sleepResult.data;
  return NextResponse.json({ displayName: profile?.display_name ?? user.displayName, dateOfBirth: profile?.date_of_birth ?? "", heightCm: Number(profile?.height_cm ?? 175), weightKg: Number(profile?.weight_kg ?? 70), importRange: profile?.import_range ?? "90_days", primaryGoal: goal.type, primaryGoalDirection: goal.direction, secondaryGoalDirections: goal.secondaryDirections, baseSleepTargetMinutes: 510, usualWakeTime: String(sleep?.usual_wake_time ?? "07:00").slice(0, 5) });
}
...
export async function PUT(request: Request) {
  ...
  // Profile details are independent of the confirmed objective managed through Soma.
  const { error } = await admin.rpc("update_soma_profile_details", {
    p_user_id: user.id,
