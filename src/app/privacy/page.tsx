...
export default function PrivacyPage() {
  return (
    <main className="legal-page" id="main-page-content">
      ...
      <article>
        <span className="eyebrow">Privacy · Version 1.3</span>
        ...
        <h2>AI processing</h2>
        <p>Soma transmits only the conversation, bounded health context, and photos you explicitly attach to the current assistant request to OpenAI. It does not resend historical conversation photos automatically and does not use external web search. Meal-analysis photos are processed separately and removed after confirmation according to the meal retention policy. Photos attached to an assistant conversation remain private in Soma storage until you delete them, the conversation, or your account. OAuth tokens and secrets are never transmitted. Assistant requests use the OpenAI Responses API with <code>store: false</code>.</p>
        <p>In Analysis, generating an AI summary is optional. When you select it, Soma sends a limited set of measured associations (metric labels, effects, delays, sample sizes and stability) to OpenAI. OpenAI only selects and orders associations; Soma displays its own measured values. Opening Analysis alone does not send these associations to OpenAI. This request also uses <code>store: false</code>.</p>
