...
- `querySomaData` : détails ciblés par période, métriques et types d’activités. Les filtres et projections sont appliqués avant pagination. Les détails des repas et des sessions de sommeil restent disponibles. Les activités consomment les pages dans l’ordre du stockage, même si sa collation diffère de celle de JavaScript.
...
Le chat signale une préparation de mémoire en attente ; la délégation vocale transmet aussi cet avertissement. Les résultats analytiques conservent des preuves structurées bornées (outil, nombre de relations, fenêtres et complétude), sans inventer une couverture datée.

Les synthèses d’historique ont un délai global de 20 secondes, dès la recherche du checkpoint, propagé aux lectures Google, au renouvellement OAuth et aux attentes de stockage. Une annulation empêche de lancer les écritures suivantes. Chaque page cardiaque est enregistrée avant sa continuation persistée. Une interruption conserve la séance non comptée et reprend sa prochaine page ; une panne de source, même sur la première page, reste explicitement en attente. `status=running` et `manifest.complete=false` désignent un résultat partiel. Le même `jobId` et la même requête permettent de reprendre après interruption ; le résultat final n’est annoncé exhaustif qu’après la dernière page. Aucun worker de traitement n’est lancé sans requête.
...
Les nouveaux timestamps sont normalisés en UTC avec millisecondes ; les payloads sources restent intacts. Pour les anciennes données avec des précisions ou offsets différents, chaque page brute relit une enveloppe de dates bornée autour de la partition demandée, puis filtre et trie les instants UTC localement. Les lectures se font par pages de 200. Un groupe de plus de 2 000 timestamps identiques ou une enveloppe live/archives dépassant 20 000 lignes produit une erreur explicite, jamais une fin de parcours artificielle. Les anciens curseurs incompatibles avec cet ordre sont rejetés : la requête doit repartir sans curseur.
