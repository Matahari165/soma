...
Les synthèses d’historique ont une durée de travail bornée par invocation. `status=running` et `manifest.complete=false` désignent un résultat partiel. Le même `jobId` et la même requête permettent de reprendre après interruption ; le résultat final n’est annoncé exhaustif qu’après la dernière page. Aucun worker de traitement n’est lancé sans requête.
