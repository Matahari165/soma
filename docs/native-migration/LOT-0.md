...
**Statut : cadrage fondé sur le dépôt au commit `0b99912` ; vérification de production ouverte.**
...
## 5. Vérification de production à terminer
...
| Photos et archives | Volumes et statuts R2, manifestes orphelins, possibilité d'export restaurable | Manifestes SQL et présence d'objets R2 vérifiés ; correspondance et restauration non vérifiées |
...
| Export | Export d'un **compte de test**, contrôle des tables et des fichiers référencés, sans versionner son contenu | Export authentifié vérifié pour profil et journal synthétique ; couverture repas, archives et photos encore à prouver |
...
Le contrôle d'un repas marqué « ignoré » révèle un écart non expliqué. L'action « Skip » de Nutrition Log a affiché une confirmation et l'état ignoré persistait après rechargement complet. Les journaux agrégés Vercel montrent un `POST /api/meals` réussi (`201`) à 09:03:28 UTC, puis un export réussi (`200`) à 09:03:51 UTC ; la catégorie `meals` de cet export était pourtant vide. La lecture du code montre que l'export ne filtre pas les repas ignorés. Ces observations écartent un export lancé avant la requête d'écriture, mais ne prouvent pas encore que les deux réponses portent sur le même enregistrement et le même compte. La lecture directe de `/api/meals` a été bloquée par le navigateur ; l'accès au projet Supabase de production n'était plus disponible lors de ce dernier contrôle. Il reste à confronter la ligne stockée, la réponse de `/api/meals` et l'export dans une même session de test avant de corriger le code. Aucun lien d'archive R2 ou de photo n'était présent sur ce compte : leur accès et leur restauration ne sont pas encore prouvés. **Le lot 0 reste ouvert sur ces preuves.**
