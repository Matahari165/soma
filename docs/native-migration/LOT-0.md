...
Le contrôle d'un repas marqué « ignoré » demande encore une clarification : l'interface conserve cet état après rechargement, tandis que la catégorie `meals` de l'export contrôlé était vide. Il faut confronter la réponse de `/api/meals` et l'export pour distinguer un état de journal d'un repas réellement enregistré avant de conclure. Aucun lien d'archive R2 ou de photo n'était présent sur ce compte : leur accès et leur restauration ne sont pas encore prouvés. **Le lot 0 reste ouvert sur ces preuves.**
