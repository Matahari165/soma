...
Quand le runtime Supabase est configuré, l'adaptateur actuel place les tables métier dans `soma_rows` ; les anciens schémas SQL typés coexistent dans les migrations. Le runtime effectivement actif en production reste à vérifier. Les applications natives ne doivent dépendre ni de `soma_rows` ni d'une table SQL directe : elles utiliseront une API versionnée. Sources : `supabase/migrations/20260913140000_soma_runtime_compatibility.sql`, `src/lib/cloudflare/db.ts`, `src/app/api/account/export/route.ts`.
...
- L'export JSON liste de nombreuses tables mais omet notamment `meal_recipe_templates` et les données logiques de compléments (`src/app/api/account/export/route.ts`). Sa couverture devra être vérifiée sur un compte de test avant de devenir le contrat d'export natif.
...
| Stockage métier | Nombre de lignes par `table_name` dans `soma_rows`, dates min/max et variantes de schéma pertinentes | À faire avec accès autorisé |
| Sources santé | Fournisseurs réellement présents, types de mesures, couverture et dernières synchronisations **agrégés** | À faire avec accès autorisé |
| Apple actuel | Existence effective du champ de jeton, du fournisseur et des données importées ; compatibilité de colonnes | À faire avec accès autorisé |
| Photos et archives | Volumes et statuts R2, manifestes orphelins, possibilité d'export restaurable | À faire avec accès autorisé |
| Traitements planifiés | Vérifier lequel du Worker Cloudflare et de `pg_cron` appelle effectivement les routes Vercel ; éviter les doubles exécutions | À faire avec accès autorisé |
| Export | Export d'un **compte de test**, contrôle des tables et des fichiers référencés, sans versionner son contenu | À faire avec compte de test |

Le code prévoit un Worker Cloudflare qui réveille les routes Vercel (`cloudflare/worker.ts`, `cloudflare/cron-pipeline.ts`) ; `supabase/setup/schedule_sync.sql` contient aussi une configuration `pg_cron`. La présence de ces fichiers ne prouve pas quel planificateur est actif. La route Apple référence des champs et un fournisseur qui ne sont pas établis par les anciennes migrations typées ; cela doit être contrôlé sur le runtime réel avant de conclure à une panne ou à une compatibilité.
