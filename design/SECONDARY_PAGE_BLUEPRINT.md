...
- La largeur du radar est bornée à `420px`. Son SVG est fluide (`width: 100%`, `height: auto`) et utilise un `viewBox` de `500 × 420` : sa géométrie est donc rectangulaire, même si son tracé est centré. Centre `(250, 210)`, rayon utile `132` unités, labels à `166` unités du centre. Le nombre d'axes dépend du domaine ; Activity en affiche quatre, uniquement les composantes du score : minutes en zone, durée d’exercice, calories actives et pas. La charge hebdomadaire reste dans la bande des indicateurs sous le radar.
