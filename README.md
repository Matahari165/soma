...
Soma is a personal laboratory that links Google Health, Google Calendar, and a 30-second daily check-in to reveal the conditions behind better focus, energy, recovery, and Deep Work.
...
The Personal Lab tests many within-person relationships but only surfaces comparisons with enough paired days, interpretable effect sizes, and reasonable direction stability. Every result is described as an association, never a cause.
...
7. Run `supabase/setup/schedule_sync.sql` after replacing its two placeholders. Supabase Cron polls the worker every five minutes. Health is contacted automatically once per civil day at 11:00 in the profile timezone; Calendar aggregates refresh roughly once a day when the health queue is idle. Both can also be refreshed explicitly.
